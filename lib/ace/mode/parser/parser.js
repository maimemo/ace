define(function (require, exports, module) {
    ...
    var Node = function (type, start, stop, data) {
        this.type = type;
        this.start = start;
        this.stop = stop;
        this.data = data;
    ...
    var transfer = function (tree, errorListener) {
        ...
        var checkAttr = function (node) {
            ...
            data.attrs.push(new Node('ATTR', node.start, node.stop, {
                key: node.children[0].symbol,
                value: node.children[2].symbol
            }));
        };

        var checkChapter = function (node) {
            data.list.push(new Node('CHAPTER', node.start, node.stop, {
                chapter: node.children[1].symbol
            }));
        };

        var checkSingleWord = function (node) {
            data.list.push(new Node('WORD', node.start, node.stop, {
                word: node.children[0].symbol
            }));
        };

        var checkVoc = function (node) {
            var voc = new Node('DEF', node.start, node.stop, {
                word: node.children[0].children[0].symbol
            });
            ...
            var checkNote = function (c) {
                var note = voc.data.note = {
                    start: c.start,
                    stop: c.stop,
                    data: {}
                };
                var noteBody = c.children.find(function (x) {return x.constructor.name === 'Note_bodyContext';});
                if (!noteBody) {
                    note.data = null;
                } else {
