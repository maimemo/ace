                ...
                if (c.children.length === 2) {
                    ...
                    if (text > MAX_INTERP_LEN) {
                        reportErr(null, c.start.line, 0, '超出最大字符数限制，超出：'
                            + (text.length - MAX_INTERP_LEN) + '个字符, 最大：' + MAX_INTERP_LEN + ' 个字符', 'error');
                    }
                    if (lineCount > MAX_INTERP_LINES) {
                        reportErr(null, c.start.line, 0, '超出最大行数限制 ' + (lineCount - MAX_INTERP_LINES) + '，最大：'
            ...
            var checkPhrase = function (c) {
                ...
                if (c.children.length === 2) {
                    ...
                    for (var i = 1; i < c.children.length - 1; i++) {
                        ...
                        if (p.length === 2) {
                            ...
                            if (p[0].length > MAX_PHRASE_EN_LEN) {
                              reportErr(null, c.start.line, 0, '超出最大字符数限制，超出：'
                                + (p[0].length - MAX_PHRASE_EN_LEN) + '个字符, 最大：' + MAX_PHRASE_EN_LEN + ' 个字符', 'error');
                            }
                            if (p[1].length > MAX_PHRASE_ZH_LEN) {
                              reportErr(null, c.start.line, 0, '超出最大字符数限制，超出：'
                                + (p[1].length - MAX_PHRASE_ZH_LEN) + '个字符, 最大：' + MAX_PHRASE_ZH_LEN + ' 个字符', 'error');
                            }
                    ...
                    if (phrase.data.phrases.length > MAX_PHRASE_COUNT) {
                        reportErr(null, c.start.line, 0, '超出最大例句数量限制, 限制 ' + MAX_PHRASE_COUNT
            ...
            var checkNote = function (c) {
                ...
                if (!noteBody) {
                    ...
                    if (noteText.length > MAX_NOTE_LENGTH) {
                      reportErr(null, c.start.line, 0, '超出最大字符数限制，超出：'
                        + (noteText.length - MAX_NOTE_LENGTH) + '个字符, 最大：' + MAX_NOTE_LENGTH + ' 个字符', 'error');
                    }
                    if (lineCount > MAX_NOTE_LINES) {
                        reportErr(null, noteBody.start.line, 0, '超出最大行数限制 ' + (lineCount - MAX_NOTE_LINES) + '，最大：'
