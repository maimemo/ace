            ...
            var checkInterp = function (c) {
                ...
                if (c.children.length === 3) {
                    ...
                    if (text > MAX_INTERP_LEN) {
                      reportErr(null, c.start.line, 0, 'Maximum number of characters exceed, max: '
                        + MAX_INTERP_LEN + ', now: ' + text.length, 'error');
                    }
                    if (lineCount > MAX_INTERP_LINES) {
                      reportErr(null, c.start.line, 0, 'Maximum number of lines exceed, max: '
                        + MAX_INTERP_LINES + ', now: ' + lineCount, 'error');
                    }
            ...
            var checkPhrase = function (c) {
                ...
                if (c.children.length === 3) {
                        ...
                        if (p.length === 2) {
                            ...
                            for (var si = 0; si < en.length; si++) {
                              if (!phraseEnRegex.test(en[si])) {
                                reportErr(null, en_line, 0, 'Character ' + en[si]
                                  + ' not allowed in english phrase', 'error');
                                break;
                              }
                            }
                            if (p[0].length > MAX_PHRASE_EN_LEN) {
                              reportErr(null, en_line, 0, 'Maximum number of characters exceed, max: '
                                + MAX_PHRASE_EN_LEN + ', now: ' + p[0].length, 'error');
                            }
                            if (p[1].length > MAX_PHRASE_ZH_LEN) {
                              reportErr(null, zh_line, 0, 'Maximum number of characters exceed, max: '
                                + MAX_PHRASE_ZH_LEN + ', now: ' + p[1].length, 'error');
                            }
                    ...
                    if (phrase.data.phrases.length > MAX_PHRASE_COUNT) {
                      reportErr(null, c.start.line, 0, 'Maximum number of phrases exceed, max: '+ MAX_PHRASE_COUNT
                        + ', now: ' + phrase.data.phrases.length, 'error');
                    }
                }
            };
            ...
            var checkNote = function (c) {
                ...
                if (!noteBody) {
                    ...
                    if (noteText.length > MAX_NOTE_LENGTH) {
                      reportErr(null, noteBody.start.line, 0, 'Maximum number of characters exceed, max: '
                        + MAX_NOTE_LENGTH + ', now: '+ noteText.length, 'error');
                    }
                    if (lineCount > MAX_NOTE_LINES) {
                      reportErr(null, noteBody.start.line, 0, 'Maximum number of lines exceed, max: '
                        + MAX_NOTE_LINES + ', now: '+ lineCount, 'error');
                    }
        ...
        var checkUnusedDefs = function() {
          ...
          data.defs.forEach(function(d) {
              if (!words[d.data.word.text]) {
                reportErr('null', d.start.line, 0, 'Unused definition: ' + d.data.word.text, 'warning');
              }
          });
        };
          ...
          data.list.forEach(function(w) {
            if (w.type === 'CHAPTER') {
              ...
              if (chapterWords[word]) {
                reportErr(word, w.start.line, 0, 'Duplicated word: ' + word + ' in chapter ' + prevChapter, 'warning');
              }
            ...
            if (lastType != null) {
                ...
                if ((type === 'ChapterContext' || type === 'Single_wordContext')
                    ...
                    reportErr(null, node.start.line, 0,
                      'Chapter or Word place only after attributes and before definitions', 'error');
                    return;
                } else {
                    lastType = type;
                }
